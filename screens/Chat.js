...
export default function Chat() {
    ...
    const senderUser = currentUser.photoURL ? {
        name: currentUser.displayName || currentUser.contactName,
        _id: currentUser.uid,
        avatar: currentUser.photoURL
    } : { name: currentUser.displayName || currentUser.contactName, _id: currentUser.uid };
        ...
        (async () => {
            if (!room) {
                const currentUserData = {
                    displayName: currentUser.displayName || currentUser.contactName,
                ...
                const userBData = {
                    displayName: userB.displayName || userB.contactName  || userB.name,
                    email: userB.email,
                }
                if (!userB.photoURL) {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(roomMessagesRef, querySanpshot => {
            const messagesFirestore = querySanpshot.docChanges()
    ...
    return (
        <ImageBackground resizeMode='cover' source={require('../assets/chatbg.png')}
            style={{ flex: 1, backgroundColor: '#EFFFFD' }}>
            <GiftedChat onSend={onSend} messages={messages}
                user={senderUser} renderAvatar={null}
                renderActions={(props) => (
                    <Actions {...props} containerStyle={{
                        position: 'absolute',
                        right: 50,
                        bottom: 5,
                        zIndex: 9999,
